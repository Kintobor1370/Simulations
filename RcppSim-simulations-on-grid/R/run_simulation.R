...
run_simulation<-
  ...
  step = 0.1
  steps_cnt = 0
  pop_steps = c(simulator$total_population)
  time_steps = c(simulator$time)
  prev_pop = simulator$total_population
  
  for(j in 2:(epochs+1)){
    total_pop <- simulator$total_population
    for(jj in 1:total_pop){
      simulator$run_events(1)
      if (simulator$time > step * (steps_cnt + 1)) {
        pop_steps <- c(pop_steps, prev_pop)
        time_steps <- c(time_steps, step * (steps_cnt + 1))
